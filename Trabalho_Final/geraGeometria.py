...
airfoil = np.array(pd.read_csv(f"{file_name}.csv", sep=','))
...
rotated_df.to_csv(f"{file_name}_{angle}.csv", index=False)
print(f"Os pontos rotacionados foram salvos no arquivo '{file_name}_{angle}.csv'")
...
with open(f'msh_{file_name}_{angle}.geo', 'w') as mesh:
    mesh.write(meshStr)

print(f"A geometria foi salva no arquivo 'msh_{file_name}_{angle}.geo'")
